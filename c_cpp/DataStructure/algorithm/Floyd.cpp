...
void randGenGraph(int n, float p, bool weighted, bool directional,
    ...
    char ch[40];
    int sz =
        sprintf(ch, "%3d\t %3d\t %5s\t %5s\n", n, ne,
                weighted ? "true" : "false", directional ? "true" : "false");
...
void printG(MGraph* g) {
    ...
    for (int i = 0; i < g->vexnum; ++i) {
        printf("ID(%u)-权(%d)-弧表 t(eid|w)：%u ", g->vertices[i].id,
               g->vertices[i].w, g->vertices[i].id);
        for (int j = 0; j < g->vexnum; j++)
