...
void merge(T a[], int p, int q, int r) {
  ...
  T* ar1 = new T[len1 + 1];
  T* ar2 = new T[len2 + 1];
  for (int i = 0; i < len1; i++) ar1[i] = a[p + i];
  for (int i = 0; i < len2; i++) ar2[i] = a[q + i + 1];
...
void heap_sort(T a[], int len) {
  ...
  temp_heap.sort();
  for (int i = 0; i < len; i++) a[i] = temp_heap[i];
}
...
// 得到数字x的第bit+1位数
inline int getbit(int x, int bit) { return x / tenp[bit] % 10; }
void radix_sort(int a[], int len, int max) {
  int d = log10(max) + 1;
  int* temp = new int[len];
  for (int i = 0; i < d; i++) {
    int count[10] = {0};
    for (int j = 0; j < len; j++) count[getbit(a[j], i)]++;
    for (int j = 1; j < 10; j++) count[j] += count[j - 1];
    for (int j = len - 1; j >= 0; j--) temp[--count[getbit(a[j], i)]] = a[j];
    for (int j = 0; j < len; j++) a[j] = temp[j];
  }
  delete[] temp;
}
