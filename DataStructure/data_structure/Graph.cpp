...
//BFS
Path SearchOnGraph_BFS(ALGraph *G, int id1, int id2, int (*Goal)(VexNode *v, int id))
{
    VexNode *s;
    //初始化访问数组
    int visit[NUM] = {0};
...
//DFS
Path SearchOnGraph_DFS(ALGraph *G, int id1, int id2, int (*Goal)(VexNode *v, int id))
{
    VexNode *s;
    //初始化访问数组
    ...
    path.vexs[0] = &G->vertices[id1];
    path.len = 1;
    int lastvex[MAX_VERTEX_NUM]; //lastvexs记录每一个顶点的上一个顶点在path.vexs中的下标
