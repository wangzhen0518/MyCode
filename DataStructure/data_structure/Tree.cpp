...
bool PostOrderTraverse(BiTree t)
{
    ...
    while (!s.empty())
    {
        ...
        p = s.top();
        if (p->rchild == nullptr && p->visited == 0) //p无右节点，且尚未访问，进行访问
        {
            p->visited = 1; //访问结点
            s.pop();        //退栈

            p = s.top();
            if (p->rchild != nullptr && p->rchild->visited == 0) //p有右节点，且右节点尚未入栈
                s.push(p->rchild);
        ...
        else if (p->rchild->visited == 1) //p有右节点，但已入过栈，则p左右节点均已访问过，对p进行访问
        {
            p->visited = 1; //访问
            s.pop();        //退栈
        }
