...
int Search_Fabon(int Fabon[], int len, SSTable* st, KeyType key) {
    if (st->elem[st->length].key < key || st->elem[0].key > key) return -1;
    //假设数据个数为斐波那契数
    //先找到数据个数对应的斐波那契数
    int i = 0;
    for (; i < len; i++) {
        if (Fabon[i] == st->length) break;
    }
    i--;
    int mid = Fabon[i - 1];
...
int Search_Insert(SSTable* st, KeyType key) {
    ...
    int low = 0;
    int mid = (key - st->elem[low].key) /
              (st->elem[high].key - st->elem[low].key) * (high - low);
    while (high >= low) {
        if (st->elem[mid].key == key)
            return mid;
        else if (st->elem[mid].key > key) {
            high = mid - 1;
            mid = (key - st->elem[low].key) /
                  (st->elem[high].key - st->elem[low].key) * (high - low);
        } else if (st->elem[mid].key < key) {
            low = mid + 1;
            mid = (key - st->elem[low].key) /
...
int Block_Search(ElemType* st, Index* ind, int n, int b, KeyType key) {
    ...
    while ((j < n) &&
           (st[j].key <=
            ind[i].maxkey))  //前一个j <
                             // n是用于防止在最后一个索引表中查找时，数据溢出
    {
        if (st[j].key == key) break;
        j++;
    }
    if ((j >= n) || (st[j].key != key)) {
        j = 0;
...
bool Creat_BBSTree(BBSTree t, KeyType key[], int len) {
    BBSTNode* q;
    for (int i = 0; i < len; i++) {
        if ((q = Insert_BBST(&t, key[i])) == NULL) return false;
        Balance(q, t);
    }
    return true;
}
...
bool InsertB_T(B_Tree* t, KeyType key, B_TNode* q, int i) {
    ...
    if(!finished)   //t是空树 (参数q初值为NULL) 或者根节点已分裂为结点*q和*qp
        NewRoot(t, q, x, ap);   //生成含信息(t, x, ap)的新的根节点*t,
    原t和ap为子树指针
    */
}
